<template>
  <!-- Section 1 -->
  <section class="py-10 bg-gradient-to-r from-pink-500 to-purple-500">
    <div class="px-10 mx-auto max-w-7xl">
      <div class="flex flex-col items-center sm:flex-row sm:justify-between">
        <a href="#_" class="flex items-center text-lg font-bold">
          <svg
            class="w-4 h-4 mr-1 text-white fill-current"
            viewBox="0 0 39 15"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M11.252 1.152C4.184 2.526.454 6.918.061 14.329c1.963-4.049 4.798-5.975 8.503-5.778 2.115.112 3.84 1.295 5.75 2.603 3.11 2.133 6.712 4.601 13.281 3.324 7.068-1.374 10.798-5.766 11.191-13.177-1.963 4.049-4.798 5.975-8.503 5.779-2.115-.113-3.84-1.296-5.75-2.604-3.11-2.133-6.712-4.601-13.281-3.324z"
              fill-rule="evenodd"
            ></path>
          </svg>
          <span class="ml-1 text-white">Health Equips</span>
        </a>
      </div>
      <div
        class="
          flex flex-col
          justify-between
          mt-5
          mb-5
          text-center
          sm:mb-0 sm:mt-12 sm:flex-row
        "
      >
        <ul
          class="
            flex flex-row
            justify-center
            pb-3
            -ml-4
            -mr-4
            text-sm
            font-bold
          "
        >
          <li>
            <a href="#_" class="px-2 text-white hover:text-black"
              >Contact</a
            >
          </li>
          <li>
            <a href="#_" class="px-2 text-white hover:text-black"
              >About US</a
            >
          </li>
          <li>
            <a href="#_" class="px-2 text-white hover:text-black"
              >FAQ's</a
            >
          </li>
          <li>
            <a href="#_" class="px-2 text-white hover:text-black"
              >Terms</a
            >
          </li>
        </ul>
        <p class="mt-3 text-xs leading-tight text-white sm:mt-0">
          © Copyright 2021
        </p>
      </div>
    </div>
  </section>
</template>

<script>
export default {};
</script>

<style>
</style>